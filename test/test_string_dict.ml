...
let%expect_test _ =
  Quickcheck.test
    (Quickcheck.String.Set.quickcheck_generator Quickcheck.String.quickcheck_generator)
    ...
    ~f:(fun strings ->
      ...
      let inputs =
        List.concat_map (Set.to_list strings) ~f:(fun str ->
          ...
          [ str
          ; str ^ "x"
          ; (if len = 0 then "foo" else String.sub str ~pos:0 ~len:(len - 1))
          ])
      in
      (let from_map = Map.to_alist map in
       ...
       require
         ...
         ~if_false_then_print_s:
           (lazy
             [%sexp { from_map : (string * int) list; from_dict : (string * int) list }]));
